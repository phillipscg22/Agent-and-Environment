...
public class SuccessorFunction implements Action {
    ...
    public static LinkedList<Node> expand(Node node, Environment environment) throws CloneNotSupportedException {
        ...
        try {
            list.add(successorFunction(parentNode, "goForward", environment));
        }
        ...
        list.add(successorFunction(parentNode, "turnRight", environment));
        parentNode = node.copyOfNode();
        list.add(successorFunction(parentNode, "turnLeft", environment));
...
//        node.addLocation(state.getLocation());
        return node;
    }
