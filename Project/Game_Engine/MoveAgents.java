...
public class MoveAgents implements Action, Percepts {
    ...
    public MoveAgents(Environment environment, int goalCells, int numOfAgents, int numOfNonPassableCells, String searchType, int depthLimit) {
            ...
            new Thread(() -> {
                ...
                try {
                    ...
                    if (truck.getMovable()) {
                        ...
                        switch(searchType) {
                            case BREADTH_FIRST_SEARCH : waitTime = 500; BreadthFirstSearch bfs = new BreadthFirstSearch(environment); goalNode = bfs.search(rootNode); break;
                            case DEPTH_FIRST_SEARCH : waitTime = 200; DepthFirstSearch dfs = new DepthFirstSearch(environment); goalNode = dfs.search(rootNode); break;
                            case DEPTH_LIMITED_SEARCH : waitTime = 400; DepthLimitedSearch dls = new DepthLimitedSearch(environment); goalNode = dls.search(rootNode, depthLimit); break;
                            case ITERATIVE_DEEPENING_DEPTH_FIRST_SEARCH : waitTime = 500; IterativeDeepeningDepthFirstSearch iddfs = new IterativeDeepeningDepthFirstSearch(environment); goalNode = iddfs.search(rootNode); break;
                            case UNIFORMED_COST_SEARCH : waitTime = 500; UniformedCostSearch ucs = new UniformedCostSearch(environment); goalNode = ucs.search(rootNode); break;
                            default : break;
                        }
