...
public class ReachedEndState {
    ...
    public static void noGoalFound(int number, String title, Node rootNode) {

        Stage window = new Stage();
        window.setTitle(title);

        StackPane stackPane = new StackPane();
        stackPane.getChildren().add(new Text("Agent " + number + " did not find a goal, they are located at " + rootNode.getState().getLocation().toString()));
