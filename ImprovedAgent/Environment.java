...
public class Environment extends GridPane {
    ...
    public Environment(double width, double height, TruckAgent [][] agents, Cell [][] cells) {
    ...
    public TruckAgent [] fillAgents(int numAgents, int numObstacles) {
        ...
        for(int i = 0; i < agents.length; i++) {
            ...
            for(int j = 0; j < agents[i].length; j++) {
                ...
                Cell cell = new Cell(false, false, true, new Location(i, j), widthOfCell, heightOfCell);
                this.add(cell, j, i);
                cells[i][j] = cell;
    ...
    public void createObstacles(int numOfObstacles) {
        ...
        while(count < numOfObstacles) {
            ...
            if((agents[i][j] == null) || (!agents[i][j].getBuilt() && agents[i][j].getPassable())) {
                ...
                Cell cell = new Cell(true, false, false, new Location(i, j), widthOfCell, heightOfCell);
                this.add(cell, j, i);
                cells[i][j] = cell;
                count++;
    ...
    public void createGoals(int numGoals) {
        ...
        while(count < numGoals) {
            ...
            if((agents[i][j] == null) || (!agents[i][j].getBuilt() && agents[i][j].getPassable())) {
                ...
                Cell cell = new Cell(true, true, true, location, widthOfCell, heightOfCell);
                cell.setGoal(true);
                this.add(cell, j, i);
                cells[i][j] = cell;
